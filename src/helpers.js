...
export const amountValidator = (amount) => +amount === 0 ? 'Amount must be greater than 0.' : '';
export const cvvValidator = (cvv) => cvv.length !== 3 ? 'CVV must contain 3 numbers.' : '';

export const isFormReady = ({CardNumber, ExpDate, Cvv, Amount}) => {
    const result = cardNumberValidator(CardNumber) + dateValidator(ExpDate) + cvvValidator(Cvv) + amountValidator(Amount);
